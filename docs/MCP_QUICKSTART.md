...
2. **Configure the plugin** (Cursor, Claude, VS Code, or GPT): Set MCP URL to `https://agentstack.tech/mcp` and your API key. Full catalog: `GET https://agentstack.tech/mcp/actions`. Tools/list advertises **`agentstack.execute`** and the Cursor-safe alias **`agentstack_execute`** (same batch payload).
